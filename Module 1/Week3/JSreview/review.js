...
//Unicode is the value each letter has. international encoding standard. example a = 97 & A = 65
var uniTeddy = "Teddy".charCodeAt(3)
console.log(uniTeddy);

// Using fromCharCode() - make it readable for us :). You'll see!

console.log(String.fromCharCode(uniTeddy))

// Take your first and last name and concat()
first = "Maurice";
last= "Jones"
fullName = first.concat(" " + last); //concatenation
console.log(fullName)

// Create a string and make it return true using startsWith()
starts = "Work";
ifStarts = starts.startsWith("W")
console.log(ifStarts);

// Now use any variable with endsWith() and return false
endsFalse = fullName.endsWith("O");
console.log(endsFalse);

// Finish the following sentence. Use includes() and return true.
const ozgur = 'Once upon a time Ozgur looked up at the Moon. When he looked at the moon he noticed that craters '
includesOzgur = ozgur.includes('Ozgur');
...
FindCow = joshHadALittleLambOopsCow.indexOf("his cow");
console.log(`Index: ${FindCow}`)
...
FindVanessa = vanessa.lastIndexOf("Vanessa")
console.log(FindVanessa);
...
replaceResult = replaceGokuWithVegeta.replace('Goku','Vegeta');
...
slicePizza = pizzaSentence.slice(6).trim();
console.log(slicePizza);

// Now using the pizza sentence, return only pizza (before the comma)
onlyPizza= pizzaSentence.slice(0,5)
...
splitNamesArray = splitTheBill.split(" ")
console.log(splitNamesArray);
...
splitByChar= splitTheBill.split("");
...
ell = basicGreeting.substring(1,4)
console.log(ell)

// Returns "JavaScript"
const ohReally = "JavaScript Substring"
javaScr= ohReally.substring(0,10)
console.log(javaScr)

// Returns aol.com
const aslDays = "xXteoWuzHereXx@aol.com"
getAol = aslDays.substring(15);
console.log(getAol);
...
let extraLove = " This is extra Love ";
console.log(extraLove.trim());
...
for (let i=0 ;i<=20;i++){
  if(i>=20){
    console.log("Print a new Sentence");
}
}
...
for (obj in whateverQueenBeySaid){
  console.log(obj);
}
...
const PS = "PS";
perscholas.forEach((person) => {console.log(person.concat(PS))})
...
console.log('Fruits:', fruitString);
...
// part1= fruits.slice(0,1)
addedFruit= fruits.slice(1,0, "lemon", "grapefruit")

// part2 = fruits.splice(1,4);
// partOneHalf = ['Lemon', 'Grapefruit']

// newFruit = part1.concat(partOneHalf).concat(part2)
...
let exoticFruits = ["Dragon Fruit", "JackFruit", "Tamarind"];
let comboFruit = addedFruit.concat(exoticFruits);
console.log(comboFruit);

// Print the last two exotic fruits without altering the newly concatenated array.

// lastTwoExotics = comboFruit.splice(6,2)
lastTwoExotics = comboFruit.slice(-2);
console.log(lastTwoExotics);
...
sortedItems = monalissaIsMessy.sort();
...
sortedMirror = mirrorMirrorOnTheWall.reverse()
console.log(sortedMirror);
...
// const newArray = words.filter(w => !w.includes("badword"));
const newArray = words.filter(words => words !== "badword")
...
const sumWithInitial = numbersToAddUp.reduce((acc, current) => acc + current
);
...
let newFruitCopy = [...addedFruit];
let connectedFruits = [...addedFruit, ...fruits]
console.log(connectedFruits);


// Using the variable with the newly connected arrays, use spread operator to add something at the end.
notFruits = ["eggs", "bread", "chicken"];

unconnectedFruits= [...connectedFruits, ...notFruits]
console.log(unconnectedFruits);

// Using the variable with the newly connected arrays, use spread operator to add something at the beginning.
veggies = ["carrots", "celery", "asparagus"]
allFoods =  [...veggies,...unconnectedFruits] 
...
function getString(strang){
  console.log(`This is my ${strang}`)
}
getString("Field");
...
const Maurice = function(firstName){
  const lastName = "Jones"
  return `${firstName} ${lastName}`;
}

console.log(Maurice("Maurice"))

// Create an arrow function that accepts a number and have it return that number doubled

// const doubletheNumber = function()
